...
def render_launcher_state(cfg: RunConfig) -> str:
    values = {
        "DESYSFLOW_LAUNCHER_SOURCE": str(cfg.source),
        "DESYSFLOW_LAUNCHER_PROJECT": cfg.project,
        "DESYSFLOW_LAUNCHER_MODEL_PROVIDER": cfg.model_provider,
        "DESYSFLOW_LAUNCHER_MODEL": cfg.model_name,
        "DESYSFLOW_LAUNCHER_BASE_URL": cfg.base_url,
        "DESYSFLOW_LAUNCHER_LANGUAGE": cfg.language,
        "DESYSFLOW_LAUNCHER_STYLE": cfg.style,
        "DESYSFLOW_LAUNCHER_CLOUD": cfg.cloud,
        "DESYSFLOW_LAUNCHER_WEB_SEARCH": cfg.web_search,
        "DESYSFLOW_LAUNCHER_ROLE": cfg.role,
        "DESYSFLOW_LAUNCHER_MODE": cfg.mode,
    }
    lines = [
        "# Auto-generated by DesysFlow CLI.",
        "# Reused by letsvibedesign for follow-up runs in the same workspace.",
    ]
...
def write_launcher_state(output_root: Path, cfg: RunConfig) -> Path:
    state_path = output_root / ".letsvibedesign-state.env"
    state_path.parent.mkdir(parents=True, exist_ok=True)
    state_path.write_text(render_launcher_state(cfg), encoding="utf-8")
    return state_path
...
def run(cfg: RunConfig) -> int:
    ...
    write_launcher_state(cfg.output_root, cfg)
